...
1. **Self-contained trust signals.** An ADL document **MUST**{:bcp14} contain enough information for a counterparty to evaluate whether to interact with the agent, without requiring access to external systems.

2. **Separation of declaration from operations.** Operational detail that changes independently of the agent's declared behavior  --  escalation contacts, audit schedules, evaluation reports, deployment logs  --  belongs in external records (e.g., a governance record in a registry), not in the passport. Profiles **MAY**{:bcp14} define linking members (e.g., `governance_record_ref`) that reference such records by stable URI.
...
- ADL documents **MUST**{:bcp14} be encoded in UTF-8.
- ADL documents **MUST**{:bcp14} be valid JSON {{RFC8259}}.
- Member names **MUST**{:bcp14} use **snake_case** (lowercase with underscores).
- All timestamps **MUST**{:bcp14} be ISO 8601 strings with timezone (e.g., `"2026-02-15T14:30:00Z"`).
- All URIs **MUST**{:bcp14} conform to {{RFC3986}}.
- **YAML authoring and JSON canonical form:** YAML is an authoring convenience; JSON is the canonical wire format. When an ADL document is authored in YAML, implementations **MUST**{:bcp14} convert it to JSON for processing and validation. The media type `application/adl+json` applies to the JSON canonical form.

## Top-Level Object

An ADL document **MUST**{:bcp14} be a single JSON object.
...
An ADL document **MUST NOT**{:bcp14} contain members not defined by this specification, a declared profile, or the extension mechanism.

## Extension Mechanism

- **Profiles:** Add domain-specific requirements and members; declared in `profiles`. See Section 13.
- **Extensions object:** Custom vendor data without a full profile. An `extensions` member **MAY**{:bcp14} appear at any object level within an ADL document. Within `extensions`, vendor data is grouped under reverse-domain namespace keys.

Vendor namespace keys **MUST**{:bcp14} use reverse-domain notation with at least two dot-separated segments (e.g., `com.acme`, `io.anthropic`, `org.example.research`). Keys **MUST**{:bcp14} conform to the `vendor-key` production in Appendix D. Single-segment keys (e.g., `acme`) and uppercase keys (e.g., `COM.ACME`) are invalid.

Implementations **MUST**{:bcp14} preserve `extensions` members when round-tripping ADL documents. Implementations **MAY**{:bcp14} ignore the contents of `extensions`. Implementations **MUST NOT**{:bcp14} reject documents containing `extensions` with unknown vendor namespaces.

The member name `extensions` is reserved at every object level in an ADL document. Implementations **MUST NOT**{:bcp14} define non-extension semantics for the `extensions` member.
...
3. **Multi-segment wildcard (`**`).** The `**` sequence matches zero or more path segments including separators. Valid only in filesystem path patterns (Section 9.3). `/data/**` matches `/data/`, `/data/foo`, and `/data/foo/bar/baz`. `**` **MUST NOT**{:bcp14} appear in host patterns, environment variable patterns, or command patterns.

4. **Restrictions.** Patterns **MUST**{:bcp14} contain wildcards only in the positions described above. Mid-string wildcards (e.g., `foo*bar`) are **NOT RECOMMENDED**{:bcp14}; implementations **MAY**{:bcp14} reject them. A bare `*` as an entire pattern (matching everything) is valid but **NOT RECOMMENDED**{:bcp14} for security-sensitive domains (`allowed_hosts`, `allowed_variables`). Implementations **SHOULD**{:bcp14} warn when a bare `*` wildcard is used in permission patterns.

Implementations **MUST**{:bcp14} apply patterns using the rules defined in this section. Implementations **MUST NOT**{:bcp14} interpret patterns as regular expressions. Formal grammar productions for pattern elements are defined in Appendix D.
...
- **REQUIRED.** Value **MUST**{:bcp14} be a string in semantic versioning format (MAJOR.MINOR.PATCH). The format **MUST**{:bcp14} conform to the `semver` production in Appendix D.
- Implementations **MUST**{:bcp14} reject documents with an unsupported `adl_spec` version.
- Implementations **SHOULD**{:bcp14} support documents with the same MAJOR version and lower or equal MINOR version.
- Pre-release suffixes (e.g., `"0.2.0-draft"`) **MUST NOT**{:bcp14} appear in `adl_spec` values. Only release versions are valid for conformance. Pre-release identifiers **MAY**{:bcp14} appear in the agent's own `version` member (Section 5.5).

Example: `"adl_spec": "0.2.0"`

## $schema

Optional. URI reference to the JSON Schema for validation. **RECOMMENDED**{:bcp14} for JSON documents (enables IDE validation). Canonical schema URI for ADL 0.2: `https://adl-spec.org/0.2/schema.json`.

## Name

Human-readable name for the agent. **REQUIRED.** Value **MUST**{:bcp14} be a non-empty string. For machine identifiers, use `id` (Section 6.1).

## Description

Human-readable description of the agent's purpose and capabilities. **REQUIRED.** Value **MUST**{:bcp14} be a non-empty string. **SHOULD**{:bcp14} be sufficient for users to understand what the agent does without examining tool definitions.

## Version

Agent's version. **REQUIRED.** Value **MUST**{:bcp14} be a string in semantic versioning format (MAJOR.MINOR.PATCH); the format **MUST**{:bcp14} conform to the `semver` production in Appendix D. Agent version changes **SHOULD**{:bcp14} follow SemVer (MAJOR: breaking; MINOR: new capabilities; PATCH: fixes, docs).

## Lifecycle

Operational lifecycle status of the agent. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object containing at minimum a `status` member.
...
**REQUIRED**{:bcp14} when `lifecycle` is present. Value **MUST**{:bcp14} be one of:
...
When `lifecycle` is omitted, no lifecycle assertion is made. Implementations **MUST NOT**{:bcp14} assume a default status.

Runtimes **SHOULD**{:bcp14} check `lifecycle.status` before provisioning agents. Runtimes **SHOULD NOT**{:bcp14} provision agents with status `draft` in production environments. Runtimes **SHOULD**{:bcp14} warn users when provisioning agents with status `deprecated`. Runtimes **MUST NOT**{:bcp14} provision or execute agents with status `retired`.
...
When present, value **MUST**{:bcp14} be a valid ISO 8601 string with timezone. Indicates when the current `status` took effect.

### sunset_date

When present, value **MUST**{:bcp14} be a valid ISO 8601 string with timezone. Indicates when the agent will be or was retired. Implementations **SHOULD**{:bcp14} warn when `sunset_date` is in the future and within 30 days. When `sunset_date` is in the past and `status` is `deprecated`, runtimes **SHOULD**{:bcp14} treat the agent as `retired`.

### successor

When present, value **MUST**{:bcp14} be a string; **SHOULD**{:bcp14} be a URI or URN identifying the replacement agent (see Section 6.1 for identifier formats). **SHOULD**{:bcp14} be present when `status` is `deprecated` or `retired`. Implementations **SHOULD**{:bcp14} warn if `successor` is present when `status` is `active` or `draft`.
...
Unique identifier for the agent. **OPTIONAL.** When present, value **MUST**{:bcp14} be a string and **MUST**{:bcp14} be a valid URI {{RFC3986}} or URN {{RFC8141}}.

Identifier formats, in order of preference:

1. **HTTPS URI (RECOMMENDED):** `https://{domain}/agents/{name}`  --  Provides ownership verification via TLS, direct resolution to the agent's ADL document, and natural integration with `.well-known` discovery (Section 6.4). The domain authority **SHOULD**{:bcp14} serve the ADL document at the identifier URL with media type `application/adl+json`.

2. **Decentralized Identifier:** `did:web:{domain}:agents:{name}`  --  Provides cryptographic identity binding via the DID Document. Resolution follows the `did:web` method specification {{W3C.DID}}. **RECOMMENDED**{:bcp14} when cryptographic verification of agent identity is required independent of transport.
...
When an agent has both a resolvable identifier (HTTPS URI or DID) and a URN, the resolvable identifier **SHOULD**{:bcp14} be used as the primary `id` value. The URN **MAY**{:bcp14} be recorded in `metadata` for catalog interoperability.

> **Note:** The `urn:adl:` namespace identifier is used as a convention in this specification but is not yet a registered URN namespace per {{RFC8141}}. Formal registration with IANA will be pursued in a future revision. Implementations **SHOULD NOT**{:bcp14} assume that `urn:adl:` URNs are globally resolvable.

## Provider

Identifies the organization or entity that provides the agent. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object:
...
Cryptographic identification for the agent. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object:
...
At least one of `did` or `public_key` **SHOULD**{:bcp14} be present. The `public_key` object, when present, **MUST**{:bcp14} contain `algorithm` (string, REQUIRED) and `value` (string, Base64-encoded, REQUIRED). Implementations **SHOULD**{:bcp14} reject weak algorithms (e.g., RSA below 2048 bits, DSA, ECDSA below P-256). EdDSA (Ed25519, Ed448) is **RECOMMENDED**{:bcp14}.
...
Agent discovery enables clients to locate agents published by a domain without prior knowledge of individual agent identifiers. Domains hosting ADL agents **MAY**{:bcp14} publish a discovery document at the well-known URI {{RFC8615}}:

```
https://{domain}/.well-known/adl-agents
```

The discovery document, when present, **MUST**{:bcp14} be a JSON object served with media type `application/json` and **MUST**{:bcp14} contain an `agents` array. Each entry in the array **MUST**{:bcp14} be an object with at least `id` (string, the agent's identifier per Section 6.1) and `adl_document` (string, URL to the full ADL document). Entries **MAY**{:bcp14} include `name`, `version`, `description`, and `status`.
...
Clients performing discovery **MUST**{:bcp14} fetch the discovery document over HTTPS. Clients **SHOULD**{:bcp14} validate the TLS certificate chain. The discovery document **SHOULD**{:bcp14} be cacheable; servers **SHOULD**{:bcp14} set appropriate `Cache-Control` headers.
...
AI model configuration. **OPTIONAL.** When omitted, the runtime determines the model. When present, value **MUST**{:bcp14} be an object:
...
System prompt for the agent. **OPTIONAL.** Value **MUST**{:bcp14} be a string or an object. When an object, it **MUST**{:bcp14} contain `template` (string, REQUIRED) and **MAY**{:bcp14} contain `variables` (object).

### Template Variable Syntax

Variables in templates use the `{{variable_name}}` syntax and **MUST**{:bcp14} conform to the `template-var` production in Appendix D. Variable names **MUST**{:bcp14} begin with a letter (`A`-`Z` or `a`-`z`) and **MAY**{:bcp14} contain letters, digits, and underscores.

**Escaping:** To include a literal `{{` in template text without triggering variable substitution, implementations **MUST**{:bcp14} support the escape sequence `\{{`. A `\{{` in the template string is rendered as `{{` and is not treated as a variable reference.

**Undefined variables:** When a template references a variable name not present in `variables`, the implementation **MUST**{:bcp14} treat this as an error (error code ADL-1006) and **MUST NOT**{:bcp14} silently substitute an empty string. Implementations **SHOULD**{:bcp14} include the undefined variable name in the error detail.
...
Array of tool objects (functions the agent can invoke). **OPTIONAL.** Each tool **MUST**{:bcp14} contain `name` (string, REQUIRED) and `description` (string, REQUIRED). Each tool **MAY**{:bcp14} contain: `parameters` (JSON Schema), `returns` (JSON Schema), `examples`, `requires_confirmation` (bool), `idempotent` (bool), `read_only` (bool), `annotations`, `data_classification` (Section 10.4). Tool names **MUST**{:bcp14} be unique, **MUST**{:bcp14} match `^[a-z][a-z0-9_]*$`, and **MUST**{:bcp14} conform to the `tool-name` production in Appendix D. The `parameters` and `returns` objects, when present, **MUST**{:bcp14} be valid JSON Schema.

The `examples` member, when present, **MUST**{:bcp14} be an array of example objects. Each example object **MAY**{:bcp14} contain:
...
The `annotations` member, when present, **MUST**{:bcp14} be an object containing implementation hints and metadata. Annotations is an open object  --  implementations **MAY**{:bcp14} add custom keys. Standard annotation members include:
...
See Section 15.3 for OpenAPI integration details. Implementations **MUST**{:bcp14} preserve all annotation members when processing, including unrecognized keys.
...
Array of resource objects (data sources the agent can access). **OPTIONAL.** Each resource **MUST**{:bcp14} contain `name` (string, REQUIRED) and `type` (string, REQUIRED). `type` **MUST**{:bcp14} be one of: `vector_store`, `knowledge_base`, `file`, `api`, `database`. Each resource **MAY**{:bcp14} contain: `description`, `uri`, `mime_types`, `schema`, `annotations`, `data_classification` (Section 10.4). Resource names **MUST**{:bcp14} be unique.

The `mime_types` member, when present, **MUST**{:bcp14} be an array of strings. Each value **MUST**{:bcp14} be a valid MIME type (e.g., `"application/json"`, `"text/plain"`).

The `schema` member, when present, **MUST**{:bcp14} be a valid JSON Schema describing the structure of the resource's data.

The `annotations` member, when present, **MUST**{:bcp14} be an object. Same semantics as `tool.annotations`  --  an open object for implementation hints that **MUST**{:bcp14} be preserved when processing.
...
Array of prompt objects (reusable prompt templates). **OPTIONAL.** Each prompt **MUST**{:bcp14} contain `name` (string, REQUIRED) and `template` (string, REQUIRED). Each prompt **MAY**{:bcp14} contain `description`, `arguments` (JSON Schema). Template arguments use `{{argument_name}}` and **MUST**{:bcp14} conform to the `template-var` production in Appendix D. Prompt names **MUST**{:bcp14} be unique.
...
The `permissions` member defines the agent's operational boundaries. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object containing one or more permission domain members.
...
Permissions operate on a **deny-by-default** model. Runtimes **MUST**{:bcp14} deny any capability not explicitly granted in the `permissions` member. Runtimes **MUST**{:bcp14} enforce declared permissions. Runtimes that cannot enforce a specific permission domain **MUST**{:bcp14} warn users before execution and **SHOULD**{:bcp14} refuse to execute the agent unless the user explicitly acknowledges the limitation.

When the `permissions` member is omitted from an ADL document, no permissions are granted to the agent. Runtimes **MUST**{:bcp14} treat the absence of `permissions` as equivalent to an empty `permissions` object  --  the agent has no granted capabilities.

When a specific permission domain (e.g., `network`, `filesystem`) is omitted from the `permissions` object, all operations in that domain are denied. For example, if `permissions` is present but does not contain `network`, the agent **MUST**{:bcp14} have no network access.

Runtimes **MUST NOT**{:bcp14} infer, assume, or provide default permissions when `permissions` or a permission domain is absent.

### Conflict Resolution

When a value matches both an `allowed_*` pattern and a `denied_*` pattern within the same permission domain, the `denied_*` pattern **MUST**{:bcp14} take precedence. The agent **MUST NOT**{:bcp14} be granted access to any value matched by a `denied_*` pattern, regardless of whether it also matches an `allowed_*` pattern. This deny-takes-precedence rule ensures that explicit exclusions cannot be overridden by broad allow patterns.
...
Host patterns in `allowed_hosts` **MUST**{:bcp14} conform to the pattern syntax defined in Section 4.4.

## Filesystem

May contain: `allowed_paths` (array of `{ path, access }` where access is `read`, `write`, or `read_write`), `denied_paths`.

Path patterns in `allowed_paths[*].path` and `denied_paths` **MUST**{:bcp14} conform to the pattern syntax defined in Section 4.4. The `**` multi-segment wildcard is valid in filesystem path patterns.

## Environment

May contain: `allowed_variables`, `denied_variables` (patterns with wildcards, e.g., `APP_*`).

Variable patterns in `allowed_variables` and `denied_variables` **MUST**{:bcp14} conform to the pattern syntax defined in Section 4.4.

## Execution

May contain: `allowed_commands`, `denied_commands`, `allow_shell` (bool).

Command patterns in `allowed_commands` and `denied_commands` **MUST**{:bcp14} conform to the pattern syntax defined in Section 4.4.
...
The `security` member defines security requirements. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object that **MAY**{:bcp14} contain `authentication`, `encryption`, and `attestation`.

## Authentication

May contain: `type` (one of `none`, `api_key`, `oauth2`, `oidc`, `mtls`), `required` (bool). Type-specific members (e.g., OAuth2: `scopes`, `token_endpoint`; OIDC: `issuer`, `audience`) **MAY**{:bcp14} be present.
...
May contain: `type` (one of `self`, `third_party`, `verifiable_credential`), `issuer`, `issued_at`, `expires_at` (ISO 8601), `signature` (object). Implementations **SHOULD**{:bcp14} warn when `expires_at` is in the past or within 30 days.

**Signature object:** When present, **MUST**{:bcp14} contain `algorithm`, `value` (Base64url-encoded), `signed_content` (`"canonical"` or `"digest"`). When `signed_content` is `"digest"`, **MUST**{:bcp14} also include `digest_algorithm` and `digest_value`. Supported algorithms include Ed25519 (RECOMMENDED), Ed448, ES256/384/512, RS256, PS256 (RSA >= 2048). Verification: remove signature, serialize with JCS {{RFC8785}}, verify digest if applicable, resolve public key from `cryptographic_identity`, verify signature.
...
The `data_classification` member declares the sensitivity and categories of data the agent may access, process, or produce. **REQUIRED.** Value **MUST**{:bcp14} be an object.
...
**REQUIRED**{:bcp14} when `data_classification` is present. Value **MUST**{:bcp14} be one of:
...
When present, **MUST**{:bcp14} be a non-empty array. Each item **MUST**{:bcp14} be one of:
...
Profiles **MAY**{:bcp14} define additional category values.

### retention

When present, **MUST**{:bcp14} be an object. **MAY**{:bcp14} contain:
...
When both `min_days` and `max_days` are present, `min_days` **MUST**{:bcp14} be less than or equal to `max_days`.
...
Profiles **MAY**{:bcp14} add domain-specific sub-objects within `data_classification` to provide granular classification vocabularies. For example, a healthcare profile may add a `healthcare` sub-object with PHI type enumerations, and a financial profile may add a `financial` sub-object with financial data type enumerations. Multiple profile extensions compose naturally within the same `data_classification` object. See Section 13 for profile composition rules.
...
The `runtime` member configures agent runtime behavior. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object.

## Input Handling

May contain: `max_input_length`, `content_types`, `sanitization`.

The `sanitization` member, when present, **MUST**{:bcp14} be an object describing input sanitization rules. It **MAY**{:bcp14} contain:
...
The `content_types` member, when present, **MUST**{:bcp14} be an array of strings. Each value **MUST**{:bcp14} be a valid MIME type specifying an accepted input content type.

## Output Handling

May contain: `max_output_length`, `format`, `streaming` (bool).

The `format` member, when present, **MUST**{:bcp14} be a string specifying the default output format. Value **MUST**{:bcp14} be one of: `"text"`, `"json"`, `"markdown"`, `"html"`.

## Tool Invocation

May contain: `parallel` (bool), `max_concurrent`, `timeout_ms`, `retry_policy`.

The `retry_policy` member, when present, **MUST**{:bcp14} be an object describing retry behavior for tool invocations. It **MAY**{:bcp14} contain:
...
The `fallback_behavior` member, when present, **MUST**{:bcp14} be an object describing behavior when errors occur and `on_tool_error` does not resolve the situation. It **MAY**{:bcp14} contain:
...
The `metadata` member provides additional information. **OPTIONAL.** When present, value **MUST**{:bcp14} be an object.

## Authors

Array of author objects. Each **MAY**{:bcp14} contain `name`, `email`, `url`.
...
Array of strings. **SHOULD**{:bcp14} be lowercase, alphanumeric and hyphens only. Tags **SHOULD**{:bcp14} conform to the `tag` production in Appendix D.
...
The `profiles` member declares which profiles the document conforms to. **OPTIONAL.** Value **MUST**{:bcp14} be an array of profile identifiers (URIs or registered names). When a profile is declared: the document **MUST**{:bcp14} satisfy all profile requirements, **MAY**{:bcp14} use profile-defined members, and validators **SHOULD**{:bcp14} check profile-specific rules.

ADL defines two categories of profiles:

- **Standard profiles** define domain-specific top-level members and validation rules. Standard profiles use the `urn:adl:profile:*` namespace and **SHOULD**{:bcp14} be registered with the IANA profile registry (Section 13.5) to prevent naming conflicts. Examples: Governance (`urn:adl:profile:governance:1.0`), Healthcare, Financial.
...
Both categories use the same `allOf` composition mechanism (Section 13.1) and **MAY**{:bcp14} appear together in a document's `profiles` array.
...
When a document declares multiple profiles, the document **MUST**{:bcp14} satisfy all declared profile requirements. Validators compose profile schemas using `allOf`  --  each profile's schema is included as an element. JSON Schema `allOf` uses "strictest wins" semantics: if any profile requires a member, the composed result requires it.

Profiles **MUST**{:bcp14} be designed for independent composition. A profile's validation rules **MUST NOT**{:bcp14} assume the absence of members defined by other profiles. For standard profiles, the IANA profile registry designated expert review (see Section 13.5) prevents cross-profile field naming conflicts. Vendor profiles avoid conflicts through their reverse-domain namespace isolation.

## Profile Dependencies

A profile **MAY**{:bcp14} declare dependencies on other profiles. When a profile declares a dependency, documents using that profile **MUST**{:bcp14} also satisfy the dependency profile's requirements. The `profiles` array **MUST**{:bcp14} include all transitive dependencies.
...
Vendor profiles use URI identifiers controlled by the vendor (e.g., `https://acme.com/adl/extensions/v1`). The `urn:adl:profile:*` namespace is reserved for standard profiles. Vendor profiles **MUST NOT**{:bcp14} use this namespace.

A vendor profile **MAY**{:bcp14} add schema constraints to the `extensions` object at any level, validating that its reverse-domain namespace contains the expected structure. The profile schema references the base ADL schema via `allOf` and declares `properties` for `extensions` within the relevant objects.

A vendor profile **MAY**{:bcp14} declare a dependency on a standard profile and add schema constraints to `extensions` within that profile's objects. The vendor profile composes its dependency via `allOf` and adds `extensions` constraints inside the profile-defined objects. This enables vendors to extend profile-defined objects without redefining them.

Vendor profiles are subject to the following constraints:

- Vendor profiles **MUST NOT**{:bcp14} redefine core ADL members or standard profile members with incompatible types.
- Vendor profiles **MUST**{:bcp14} only add schema constraints within their own reverse-domain namespace under `extensions`.
- A vendor profile's `extensions` schema applies only when the vendor profile is declared in the document's `profiles` array.
- Documents **MAY**{:bcp14} include `extensions` data for a vendor without declaring the vendor's profile. In this case, the data is preserved but unvalidated  --  implementations treat it as opaque.
...
## Standard Profile Registration

Standard profile identifiers **SHOULD**{:bcp14} be registered to prevent naming conflicts. Only standard profiles  --  those using the `urn:adl:profile:*` namespace  --  are subject to registration. Vendor profiles rely on reverse-domain namespace isolation and do not require registration (see Section 13.4).

The registration authority (e.g., IANA profile registry) **MUST**{:bcp14} employ designated expert review to ensure:
...
If a member becomes cross-cutting (needed by multiple standard profiles), the registration authority **MAY**{:bcp14} recommend promoting it to the core ADL specification.
...
Implementations **MUST**{:bcp14} parse ADL as JSON {{RFC8259}}, **MUST**{:bcp14} reject invalid JSON, and **MUST**{:bcp14} reject documents where the top-level value is not a JSON object.

## Validation

Implementations **MUST**{:bcp14} validate ADL documents against the JSON Schema defined in Appendix A. Implementations **MUST**{:bcp14} validate the following semantic rules:
...
Implementations **MAY**{:bcp14} perform additional validation based on declared profiles.

## Unknown Members

Implementations **MUST**{:bcp14} preserve unrecognized members when round-tripping. Implementations **MUST NOT**{:bcp14} reject documents containing `extensions` with unknown vendor namespaces. Implementations **MAY**{:bcp14} warn on unknown non-extension, non-profile members.
...
Implementations **SHOULD**{:bcp14} support generating A2A Agent Cards from ADL (e.g., name, description, version, tools->skills, cryptographic_identity.did->id, security.authentication->authentication).

## MCP Server Configuration

Implementations **SHOULD**{:bcp14} support generating MCP server configurations (name, description, version, tools, resources, prompts).

## OpenAPI Integration

Tools that invoke HTTP APIs **MAY**{:bcp14} reference OpenAPI specs. The tool `annotations` object **MAY**{:bcp14} contain `openapi_ref` (URI) and `operation_id`.
...
Implementations **SHOULD**{:bcp14} return errors in a consistent format, e.g.:
...
The `source` object **MAY**{:bcp14} contain: `pointer` (JSON Pointer to the error location), `line` (1-indexed), `column` (1-indexed).
...
- **Optional parameters:**
  - `profile`  --  A comma-separated list of ADL profile identifiers (URIs or registered names from the ADL Profile Registry defined in Section 17.2) that the document conforms to. Each identifier **MUST**{:bcp14} be a URI conforming to {{RFC3986}}. Consumers that do not recognize a profile identifier **MAY**{:bcp14} ignore the parameter and **MUST**{:bcp14} preserve it when retransmitting the document.
- **Encoding considerations:** binary  --  ADL documents are JSON text sequences encoded in UTF-8 {{RFC8259}}. No other character encoding is permitted. Consistent with {{RFC8259}}, UTF-8 without a byte-order mark (BOM) is **RECOMMENDED**{:bcp14}.
- **Security considerations:** ADL documents declare agent behavior including permission grants, system prompt templates, tool invocation configuration, and cryptographic identity. Processors **MUST**{:bcp14} treat content from untrusted sources with appropriate caution. Template variables in `system_prompt` and prompt templates use a `{{variable_name}}` substitution syntax; processors **MUST**{:bcp14} sanitize variable values before substitution to prevent prompt injection attacks that could alter agent behavior. ADL documents include URI references in fields such as `$schema`, `openapi_ref`, `documentation`, and `repository`; processors **MUST NOT**{:bcp14} automatically dereference these URIs from untrusted documents, as doing so may target internal network resources and enable Server-Side Request Forgery (SSRF). Documents that declare broad permissions (e.g., a bare `*` wildcard in `allowed_hosts`) represent elevated risk and **SHOULD**{:bcp14} require explicit human review before deployment. Processors **SHOULD**{:bcp14} impose limits on document size, JSON nesting depth, and array lengths to prevent resource exhaustion from adversarially crafted documents. For a comprehensive treatment of all security considerations applicable to this media type, see Section 18.
- **Interoperability considerations:** ADL documents **MUST**{:bcp14} be processed as JSON {{RFC8259}} regardless of authoring format. YAML is a common authoring convenience, but processors **MUST**{:bcp14} operate on the JSON form; documents intended to be signed using JCS {{RFC8785}} **MUST**{:bcp14} be serialized as JSON before signing. Profile declarations  --  whether via the `profile` optional parameter or the `profiles` document member  --  allow multiple profiles to compose within a single document; consumers that partially implement profile requirements **SHOULD**{:bcp14} process the members they recognize and preserve unrecognized members per Section 14.3. Validation against the JSON Schema defined in Appendix A provides a baseline interoperability check. Implementations that generate A2A Agent Cards or MCP server configurations from ADL documents **SHOULD**{:bcp14} follow the mappings defined in Section 15. Producers **SHOULD**{:bcp14} include the `$schema` member to enable tooling-assisted validation.
...
- **Fragment identifier considerations:** Fragment identifiers for resources of this type **SHOULD**{:bcp14} be interpreted as JSON Pointer expressions {{RFC6901}} identifying a location within the ADL document object.
...
**Registration Template:** Parties wishing to register a profile **MUST**{:bcp14} provide all of the following fields:

| Field | Description |
|-------|-------------|
| Identifier (URI) | A URI that uniquely identifies the profile, conforming to {{RFC3986}}. The URI **SHOULD**{:bcp14} be dereferenceable and return a human-readable description of the profile. |
...
| Specification Reference | A stable, publicly accessible URI or document reference for the profile specification. The specification **MUST**{:bcp14} define all profile-required members, validation rules, and any additional semantics added by the profile. |
...
**Designated Expert Criteria:** The designated expert **SHOULD**{:bcp14} evaluate requests against the following criteria:

1. **Publicly available specification:** The profile specification **MUST**{:bcp14} be accessible at a stable, public URI. Specifications behind paywalls or access controls are not acceptable for registration.
2. **Non-conflict with core ADL:** The profile **MUST NOT**{:bcp14} redefine or contradict normative requirements of the core ADL specification. Profiles **MAY**{:bcp14} add new members, constrain optional members to a subset of permitted values, or require that optional core members be present.
3. **Complete registration template:** All required template fields **MUST**{:bcp14} be present and non-empty. Incomplete registrations **MUST**{:bcp14} be returned to the submitter.
4. **Stable identifier:** The profile URI **SHOULD**{:bcp14} be dereferenceable and **SHOULD**{:bcp14} remain stable over time. Ephemeral or frequently changing URIs are not acceptable.
5. **Legitimate purpose:** The profile **SHOULD**{:bcp14} address a genuine domain or deployment need not already covered by an existing active registered profile.
...
- **Security and Privacy:** URN identifiers in this namespace are opaque strings and carry no inherent security properties. Implementations **MUST NOT**{:bcp14} infer ownership, trust, or authorization from a `urn:adl:` identifier alone. Verification of agent identity **MUST**{:bcp14} rely on the mechanisms described in Section 6.3 (Cryptographic Identity) and Section 10.3 (Attestation). See Section 18 for comprehensive security considerations.
...
- **Status:** permanent
- **Related information:** The well-known URI `https://{domain}/.well-known/adl-agents` returns a JSON document listing all ADL agents published by the domain authority. The document format is defined in Section 6.4. The resource **MUST**{:bcp14} be served over HTTPS.
...
ADL documents define agent behavior, permission grants, and security requirements. The trust model for an ADL document depends on its provenance and the integrity mechanisms applied to it. Unsigned ADL documents from untrusted or unverified sources **MUST**{:bcp14} be treated as potentially malicious.

When a document includes a cryptographic signature in `security.attestation.signature`, implementations **MUST**{:bcp14} verify the signature before acting on the document's permission or security declarations. Signature verification requires serializing the document (with the signature object removed) using JCS {{RFC8785}} to produce a canonical byte sequence, then verifying the resulting digest using the algorithm and public key declared in `cryptographic_identity`. Implementations **MUST**{:bcp14} reject documents that claim to be signed but whose signature does not verify. Implementations **SHOULD**{:bcp14} warn when processing signed documents whose attestation has expired (`expires_at` is in the past). An ADL document whose permissions or capabilities have been modified after signing will produce a different canonical byte sequence and fail signature verification; this is the intended behavior and provides protection against privilege escalation via document tampering.

## Sensitive Data in ADL Documents

ADL documents **SHOULD NOT**{:bcp14} contain secrets, credentials, or other sensitive data in plaintext. Fields such as `system_prompt`, `provider.contact`, `metadata.authors`, and tool parameter examples may inadvertently expose confidential information if documents are logged, cached, or transmitted without adequate access controls.

API keys, passwords, private keys, bearer tokens, and other authentication material **MUST NOT**{:bcp14} appear as literal string values in ADL documents. Where agent configuration requires secret values at runtime, implementations **SHOULD**{:bcp14} use environment variable references or external secret manager URIs rather than embedding values directly. Implementations **SHOULD**{:bcp14} warn when string values match patterns commonly associated with credentials (e.g., values matching the format of known API key prefixes). Organizations **SHOULD**{:bcp14} subject ADL documents to the same secret-scanning controls applied to source code repositories before storage or distribution.
...
Implementations **MUST**{:bcp14} sanitize template variable values before substitution. At minimum, implementations **SHOULD**{:bcp14} escape or reject values that contain the template delimiter sequence `{{` or `}}`, and **SHOULD**{:bcp14} apply length limits to variable values. Applications that allow end users to supply template variable values **SHOULD**{:bcp14} treat such values as untrusted and apply content validation appropriate to the deployment context. Runtimes operating on agents with `data_classification.sensitivity` of `confidential` or `restricted` **SHOULD**{:bcp14} log rendered prompts (after variable substitution) to enable post-incident review, subject to applicable privacy constraints.
...
ADL documents intended for public distribution **SHOULD**{:bcp14} be reviewed to remove or generalize infrastructure-specific information. Host patterns **SHOULD**{:bcp14} use registered domain names rather than IP addresses or internal hostnames. Path patterns **SHOULD**{:bcp14} avoid exposing sensitive directory names. Documents with `data_classification.sensitivity` of `confidential` or `restricted` **SHOULD**{:bcp14} only be distributed to parties with appropriate access authorization and **SHOULD NOT**{:bcp14} be published to public registries without thorough review.
...
A bare `*` as the sole value of an entry in `allowed_hosts` grants access to all hostnames and effectively disables network permission enforcement. Implementations **MUST**{:bcp14} warn when a bare `*` wildcard is used in any security-sensitive permission pattern, including `allowed_hosts` and `allowed_variables`. Implementations **SHOULD**{:bcp14} require explicit user acknowledgment  --  or refuse to deploy  --  agents that use bare `*` patterns in these domains. Pattern evaluation **SHOULD**{:bcp14} be bounded in time and space: implementations that use backtracking pattern matchers **SHOULD**{:bcp14} reject or normalize patterns that would require exponential backtracking (e.g., consecutive wildcards such as `***`). The `**` multi-segment wildcard **MUST NOT**{:bcp14} appear in host, environment, or command patterns, and implementations **MUST**{:bcp14} reject documents in which it does.

## URI Reference Attacks (SSRF)
...
Implementations **MUST NOT**{:bcp14} automatically dereference URI values from ADL documents received from untrusted sources without explicit operator or user consent. Implementations that fetch external schema documents (e.g., via `$schema`) for validation purposes **SHOULD**{:bcp14} use an allowlist of trusted schema hosts and **MUST NOT**{:bcp14} follow redirects that leave the trusted set. When fetching `openapi_ref` documents for tool description or validation, implementations **SHOULD**{:bcp14} verify that the target URI matches a pre-approved allowlist. Implementations **SHOULD**{:bcp14} validate that URI values in ADL documents conform to {{RFC3986}} and **SHOULD**{:bcp14} reject URIs with schemes other than `https`, `http`, or `urn` unless the deployment context explicitly allows them.
...
Implementations **MUST**{:bcp14} use a conformant JCS {{RFC8785}} implementation for both signing and verification. Implementations **SHOULD**{:bcp14} validate their JCS implementation against the test vectors provided in RFC 8785 before use in a production environment. Implementations **MUST NOT**{:bcp14} verify signatures against non-canonical serializations such as pretty-printed JSON or YAML. Implementations that process ADL documents containing IEEE 754 floating-point values in signed content **SHOULD**{:bcp14} be aware that platform-specific floating-point representation differences may affect canonicalization and **SHOULD**{:bcp14} avoid floating-point values in fields that will be signed when possible.
...
Publishers **SHOULD**{:bcp14} review ADL documents for personally identifiable information (PII) before public distribution and **SHOULD**{:bcp14} use organizational or role-based contact addresses rather than personal email addresses. Implementations that log ADL document contents for debugging or auditing **SHOULD**{:bcp14} redact or omit `provider.contact`, `metadata.authors`, and `system_prompt` fields from logs unless there is a documented operational requirement to retain them. Users **SHOULD**{:bcp14} be informed when their ADL documents are transmitted to third-party services for validation, indexing, or registry queries.
...
Implementations **SHOULD**{:bcp14} verify document integrity (Section 10.3) before enforcing the permissions declared in a document, particularly when documents are retrieved from network locations, shared storage systems, or public registries. Runtimes that cannot verify document integrity **SHOULD**{:bcp14} apply compensating controls  --  such as mandatory human review  --  before deploying agents that declare elevated permissions or sensitive data access. When a document's `data_classification.sensitivity` is `confidential` or `restricted`, runtimes **SHOULD**{:bcp14} require a verified signature or a verified supply chain (e.g., document retrieved from a trusted registry over an authenticated and integrity-protected channel) before provisioning. Organizations **SHOULD**{:bcp14} maintain an inventory of approved ADL documents along with their expected signatures or cryptographic digests, and **SHOULD**{:bcp14} treat any discrepancy between the recorded and observed document as a potential security incident.

## Cross-Origin and Supply Chain Concerns
...
ADL documents **SHOULD**{:bcp14} be fetched over authenticated, integrity-protected channels (HTTPS with full certificate validation). Implementations **SHOULD**{:bcp14} verify document signatures (Section 10.3) when documents are retrieved from remote or third-party sources. Implementations **SHOULD**{:bcp14} validate that the signing identity declared in `cryptographic_identity` matches an expected, trusted identity for the document's declared `provider`.

Supply chain integrity requires attention at every reference boundary: the ADL document itself, referenced OpenAPI specifications (`openapi_ref`), and external JSON Schemas (`$schema`). Implementations that automatically resolve external references during provisioning **SHOULD**{:bcp14} pin or verify all such references. When accepting ADL documents from third-party sources, implementations **SHOULD**{:bcp14} apply an allowlist of trusted providers (based on `provider.name` or `id` URI authority), verify attestation signatures from trusted issuers, and treat documents from unverified sources with the same caution applied to untrusted executable code.

## Permission Model and Defense in Depth
...
Runtimes **MUST**{:bcp14} enforce declared permissions and **MUST NOT**{:bcp14} allow agents to exceed those permissions under any circumstances, including error conditions or fallback behaviors. Runtimes that cannot enforce a specific permission domain (e.g., because the underlying platform lacks the required isolation primitives) **MUST**{:bcp14} warn users before execution and **SHOULD**{:bcp14} refuse to execute the agent unless the user explicitly acknowledges the limitation.

Beyond permission enforcement, runtimes **SHOULD**{:bcp14} monitor agent behavior during execution: logging tool invocations, recording network destinations contacted, and alerting on anomalous activity such as repeated attempts to access resources outside declared permissions. The ADL document represents intended access boundaries at definition time; runtime monitoring ensures actual behavior remains within those boundaries in production.

Runtimes **SHOULD**{:bcp14} validate tool inputs and outputs against the declared JSON Schema (Section 8.1) before passing them to or from the agent. Malformed responses from external tool implementations could inject unexpected data into agent reasoning; runtime-level schema validation provides a defense against malfunctioning or malicious tool backends. Tools annotated with `requires_confirmation: true` **MUST**{:bcp14} receive explicit user confirmation before invocation; runtimes **MUST NOT**{:bcp14} invoke such tools autonomously regardless of other configuration.

Lifecycle status **MUST**{:bcp14} be enforced as a security boundary. Runtimes **MUST NOT**{:bcp14} provision or execute agents with `lifecycle.status` of `retired`. Retired agents may have revoked credentials, unpatched vulnerabilities, or stale permission configurations. Agents with `lifecycle.status` of `deprecated` **SHOULD**{:bcp14} trigger warnings to operators, who **SHOULD**{:bcp14} migrate to the agent identified by `lifecycle.successor` before the `sunset_date` is reached.
