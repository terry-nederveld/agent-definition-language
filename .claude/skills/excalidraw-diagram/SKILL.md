...
## Output Location (where diagrams go)

**Save every diagram in `versions/draft/diagrams/` unless the user explicitly says otherwise.** This is the version-pinned home for spec and use-case diagrams (see `versions/draft/diagrams/README.md`); do not ask the user where to put it, and do not scatter `.excalidraw` files elsewhere in the repo.

For a diagram named `<name>`:

- Editable master → `versions/draft/diagrams/<name>.excalidraw` (source of truth)
- Exported render → `versions/draft/diagrams/<name>.svg`, with a **white background baked in** (Export image → SVG, Background on, dark mode off) so the brand colors stay readable on the site's white diagram card in both light and dark themes
- Commit **both** the `.excalidraw` master and the `.svg` together

Name the file after what it explains — e.g. a diagram for `versions/draft/use-cases/multi-hop-authorization.md` is `versions/draft/diagrams/multi-hop-authorization.excalidraw`. See `versions/draft/diagrams/README.md` for how docs embed the SVG (relative paths, never `/img/...`).
